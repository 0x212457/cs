...
CS_WEB_SITE_URL=http://cs.bl.lc/
CS_WEB_SITE_DIR=/var/www/cs.bl.lc
CS_WEB_ADOC_FILE=$CS_WEB_SITE_DIR/index.adoc
CS_WEB_HTML_FILE=$CS_WEB_SITE_DIR/index.html
CS_WEB_LOG_FILE=$CS_WEB_SITE_DIR/index.log

inotifywait -m $CSGOSRV_HOME_DIR -e close_write | while read path action file
                        do
                                if [[ "$file" =~ .*dem$ ]] # Does the file end with .dem?
                                        then
                                                ln -s ${path}${file} $CS_WEB_SITE_DIR
                                                FILE_SIZE=`du -hL ${path}${file} | cut -f1`
                                                #echo -e " " >> $CS_WEB_ADOC_FILE
                                                echo -e "${CS_WEB_SITE_URL}${file}[$file, $FILE_SIZE]" >> $CS_WEB_ADOC_FILE
                                                echo -e " " >> $CS_WEB_ADOC_FILE
